...
JWTKeychainProvider uses the default mailer as configured in `mail.json` or `mailgun.json` for sending password reset emails.
...
When a users requests a password reset (as a `POST` to `/users/reset-password/request`), they receive an email with a link (`/users/reset-password/form`) to a web form. That link contains a password reset token in the query that is valid only for a limited time and is verified when performing the password reset (`POST` to `/users/reset-password/change`). As an extra confirmation (in the default implementation) the user is asked to input their email address in the form.

#### Token Generator Command
In order to generate password reset tokens for users manually, add the following to `droplet.json`'s `commands`: `"keychain:generate_token"`. Then you can create a token like so:
...
An easy way to get started is to copy the existing User implementation (ie. `User.swift`, `UserForm.swift`, and `ResetPasswordForm`, adding `import JWTKeychain` as necessary) to your project and adapt to your needs.
